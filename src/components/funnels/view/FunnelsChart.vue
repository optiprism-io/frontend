<template>
  ...
  >
    ...
    >
      <div v-for="(_, i) in stepIterator" :key="i" class="pf-m-flex-1 pf-m-spacer-none">
        <FunnelChartStacked
          :data="data[i]"
          :width="stepWidth"
          :colors="barsColors"
...
const stepNumbers = computed((): number[] => {
  const metricValueColumns = props.reports.filter(col => col.type === 'metric')
...
const dimensions = computed((): string[] => {
  const result: string[] = []
  const columns = props.reports.filter(col => col.type === 'dimension')
...
const conversionCount = computed((): number[][] => {
  const columns = props.reports.filter(col => col.name === 'conversionCount')
  return convertColumns(columns, stepNumbers.value)
})

const conversionRatio = computed((): number[][] => {
  const columns = props.reports.filter(col => col.name === 'conversionRatio')
  return convertColumns(columns, stepNumbers.value)
})

const dropOffCount = computed((): number[][] => {
  const columns = props.reports.filter(col => col.name === 'dropOffCount')
  return convertColumns(columns, stepNumbers.value)
})
