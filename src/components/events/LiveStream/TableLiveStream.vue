<template>
  <div class="table-live-steam pf-u-min-height" style="--pf-u-min-height--MinHeight: 24ch">
    ...
    >
      ...
      <template #after>
        <div v-if="liveStreamStore.columnsMap.length" class="pf-c-toolbar__item pf-u-ml-auto">
          <Select
            :items="selectColumns"
          ...
          >
            <UiButton class="pf-c-select__toggle" type="button">
              {{ columnsButtonText }}
...
const tableColumnsValues = computed(() => {
  return [
    ...
    ...liveStreamStore.columnsMap
      .filter(key => liveStreamStore.activeColumns.includes(key))
      .map(key => {
        return {
          value: key,
  ...
  return liveStreamStore.reports.map((data: Report) => {
    return tableColumnsValues.value.map((column: Column): Cell | EventCellType => {
      if (liveStreamStore.defaultColumns.includes(column.value)) {
        const value =
          column.value === 'eventName'
            ? data.name
            : getStringDateByFormat(String(data.properties[column.value]), '%d %b, %Y')
        ...
        const cell: Cell | EventCellType = {
          ...
          customEvents:
            column.value === customEvents &&
            lexiconStore.customEvents?.length &&
            Array.isArray(data.matchedCustomEvents)
              ? data.matchedCustomEvents.map(event => {
        ...
        const value =
          column.value in data.properties
            ? data.properties[column.value]
            : data.userProperties && column.value in data.userProperties
              ? data.userProperties[column.value]
              : ''
